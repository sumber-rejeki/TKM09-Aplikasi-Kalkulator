...
//    }
    ...
    private fun onEqualsClicked() {
        if (inputValue1 != null && inputValue2 != null && currentOperator != null) {
            // Lakukan perhitungan jika semua nilai telah diatur
            result = calculate()
            // Tampilkan hasil perhitungan di layar
            updateResultOnDisplay()
            // Set ulang semua nilai untuk persiapan perhitungan berikutnya
            inputValue1 = result
            inputValue2 = null
            currentOperator = null
        }else {
    ...
    private fun getFormattedDisplayValue(value: Double?): String? {
        val originValue = value ?: return null
        return if (originValue % 1 == 0.0){
            originValue.toInt().toString()
        }else{
